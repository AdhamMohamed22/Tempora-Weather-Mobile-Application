...
class HomeScreenViewModel(private val repository: Repository) : ViewModel() {
    ...
    class HomeScreenViewModelFactory(private val repository: Repository) :
    ...
    private val mutableCurrentWeather =
        MutableStateFlow<CurrentWeatherResponseState>(CurrentWeatherResponseState.Loading)
    val currentWeather = mutableCurrentWeather.asStateFlow()

    private val mutableMessage = MutableSharedFlow<String>()
    val message = mutableMessage.asSharedFlow()

    private val mutableTodayForecastWeather =
        MutableStateFlow<ForecastWeatherResponseState>(ForecastWeatherResponseState.Loading)
    val todayForecastWeather = mutableTodayForecastWeather.asStateFlow()

    private val mutable5DaysForecastWeather =
        MutableStateFlow<ForecastWeatherResponseState>(ForecastWeatherResponseState.Loading)
    ...
    fun getCurrentWeather(lat: Double, lon: Double, context: Context) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            try {
                val result =
                    repository.getCurrentWeather(lat, lon, BuildConfig.appidSafe, units, language)
    ...
    fun getTodayForecastWeather(lat: Double, lon: Double, context: Context) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            try {
                val result =
                    repository.getForecastWeather(lat, lon, BuildConfig.appidSafe, units, language)
    ...
    private fun insertCashedWeather(
        cashedCurrentWeather: CurrentWeather?,
        cashedForecastWeather: ForecastWeather?
    ) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                // Store cashed values
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                if (cachedWeather != null) {
                    ...
                    mutableCurrentWeather.value =
                        CurrentWeatherResponseState.Success(cachedWeather.currentWeather)
                    ...
                    mutable5DaysForecastWeather.value =
                        ForecastWeatherResponseState.Success(filteredForecast)

                    mutableMessage.emit(context.getString(R.string.network_connection_lost))
                } else {
                    mutableMessage.emit("No cached weather available!")
                    mutableCurrentWeather.value =
                        CurrentWeatherResponseState.Failed(Exception("No cached data"))
                }
            } catch (ex: Exception) {
