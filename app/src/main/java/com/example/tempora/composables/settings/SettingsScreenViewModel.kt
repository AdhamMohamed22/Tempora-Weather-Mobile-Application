...
class SettingsScreenViewModel(private val preferencesManager: PreferencesManager) : ViewModel() {
    ...
    class SettingScreenViewModelFactory(private val preferencesManager: PreferencesManager) :
    ...
    private fun loadPreferences() {
        ...
        viewModelScope.launch {
            preferencesManager.getPreference(PreferencesManager.TEMPERATURE_UNIT_KEY, "Kelvin °K")
                .collect {
                    mutableSelectedTemperatureUnit.value = it
                }
        }
        viewModelScope.launch {
            preferencesManager.getPreference(PreferencesManager.WIND_SPEED_UNIT_KEY, "Meter/Sec")
                .collect {
    ...
    fun savePreference(key: Preferences.Key<String>, value: String, context: Context) {
        viewModelScope.launch {
            ...
            if (key == PreferencesManager.LANGUAGE_KEY) {
                val languageCode =
                    if (value == "Arabic" || value == "العربية") "ar" else if (value == "English" || value == "الإنجليزية") "en" else if (value == "Default" || value == "الافتراضية") "" else "en"
            ...
            when (key) {
                PreferencesManager.TEMPERATURE_UNIT_KEY -> {
                    val automaticWindSpeedUnit =
                        if (value == "Celsius °C" || value == "سيلسيوس °C" || value == "Kelvin °K" || value == "فهرنهايت °F") {
                            "Meters/Sec"
                        } else {
                            "Miles/Hour"
                        }
                    preferencesManager.savePreference(
                        ...
                        automaticWindSpeedUnit
                    )
                    mutableSelectedWindSpeedUnit.value = automaticWindSpeedUnit
                }

                PreferencesManager.WIND_SPEED_UNIT_KEY -> {
                    val automaticTemperatureUnit =
                        if (value == "Meters/Sec" || value == "متر/ثانية") {
            ...
            if (key == PreferencesManager.LOCATION_KEY) {
                mutableSelectedLocation.value =
                    if (value == "Map" || value == "الخريطة") "Map" else "GPS"
            }
        }
    }
